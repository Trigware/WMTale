...
func print_sequence(base_key, variables: Dictionary, preset := Preset.Fallback):
	var text_index = 1
	while true:
		var current_text_key = base_key + str(text_index)
		if not Localization.text_exists(current_text_key):
			break
		await print_wait_localization(current_text_key, variables, preset)
		text_index += 1
